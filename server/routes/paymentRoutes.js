  ...
  try {
    const [rows] = await db.query(`
      SELECT
        ...
        b.totalAmount, b.status AS bookingStatus, b.paymentMethod,
        v.make, v.model, v.year,
        vi.imageUrl,
        p.id AS paymentId, p.amount, p.method, p.status AS paymentStatus
      FROM Bookings b
...
router.post("/process/:bookingId", async (req, res) => {
  ...
  try {
    ...
    // 1. Get booking + user email for confirmation
    const [bookings] = await connection.query(`
      SELECT b.id, b.totalAmount, b.confirmationNumber,
             b.startDate, b.endDate, b.paymentMethod,
             v.make, v.model, v.year,
             u.email, u.firstName,
    ...
    await connection.query(`
      UPDATE Payments SET status = 'completed', transactionId = ?, processedAt = NOW()
      WHERE bookingId = ?
    ...
    if (booking.email) {
      const startFormatted = new Date(booking.startDate).toLocaleDateString('en-US', { weekday:'long', year:'numeric', month:'long', day:'numeric' });
      const endFormatted   = new Date(booking.endDate).toLocaleDateString('en-US',   { weekday:'long', year:'numeric', month:'long', day:'numeric' });
          ...
          <div style="font-family:Arial,sans-serif;background:#050508;color:#f0f2f8;padding:40px;max-width:560px;margin:0 auto;border-radius:6px;">
            ...
            <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
              ${[
                ...
                ['Pick-up',    startFormatted],
                ['Drop-off',   endFormatted],
                ['Location',   booking.pickupLocation || 'See confirmation'],
                ['Payment',    booking.paymentMethod],
                ['Total Paid', `$${Number(booking.totalAmount).toFixed(2)} USD`],
                ['Transaction',transactionId],
              ].map(([label, value]) => `
                <tr>
                  <td style="padding:10px 0;border-bottom:1px solid #1e1e2e;color:#4a5060;font-size:11px;letter-spacing:2px;text-transform:uppercase;width:140px;">${label}</td>
                  <td style="padding:10px 0;border-bottom:1px solid #1e1e2e;color:#f0f2f8;font-size:13px;">${value}</td>
                </tr>
              `).join('')}
            </table>
            ...
            <p style="color:#4a5060;font-size:12px;line-height:1.8;margin-bottom:24px;">
              ...
              If you need to cancel or modify your reservation, please log in to your account at least 24 hours before your pick-up date.
            </p>

            <div style="border-top:1px solid #1e1e2e;padding-top:16px;">
              <p style="color:#3a3a50;font-size:11px;margin:0;">© 2026 Rental 10 · Syracuse, NY · This is an automated message, please do not reply.</p>
            </div>
          </div>
        `,
